...
let snake = {
    ...
    makeStep() {
        ...
        var nextHeadPoint = this.getNextStepHeadPoint();
        if (nextHeadPoint.x > settings.colsCount){
            ...
            this.body.pop();
        } else if (nextHeadPoint.x < -1){
            this.body.unshift({x: settings.colsCount, y: this.body[0].y});
            this.body.pop();
        } else if (nextHeadPoint.y < -1){
            this.body.unshift({x: this.body[0].x, y: settings.rowsCount});
            this.body.pop();
        }
    },
...
let renderer = {
    ...
    render(snakePointArray, foodPoint, barrierPoint) {
        ...
        if (!status.isStopped()){
            this.cells[`x${barrierPoint.x}_y${barrierPoint.y}`].classList.add('barrier');
        }
    }
};
...
let game = {
    ...
    reset() {
        ...
        setInterval(() => this.barrier.setBarrierCoordinates(this.getRandomCoordinates()), 15000 / this.settings.speed);

        this.render();
    },

    render() {
        this.renderer.render(this.snake.body, this.food.getFoodCoordinates(), this.barrier.getBarrierCoordinates());
    },
    ...
    play() {
        ...
        this.tickInterval = setInterval(() => this.tickHandler(), 1000 / this.settings.barrierSpeed)
    ...
    tickHandler() {
        ...
        if (this.barrier.isBarrierPoint(this.snake.getNextStepHeadPoint())) {
            this.finish();
            this.barrier.setBarrierCoordinates(this.getRandomCoordinates());
        }

        gameStistic.setFoodStat();
        this.snake.makeStep();
        this.render();
    },
    ...
    getRandomCoordinates() {
        ...
        //с оператором spread
        //[{x: 5, y: 5}, {x: 5, y: 5}, {x: 5, y: 5}, {x: 5, y: 5}]
        let exclude = [this.food.getFoodCoordinates(), this.barrier.getBarrierCoordinates(), ...this.snake.body];
        console.log(exclude);
        while (true) {
...
var gameStistic = {
    ...
    renderFoodCount(){
        ...
        var divStat = document.getElementById('stat');
        var stat = document.createElement('span');
        stat.innerHTML = 'Ваш рекорд: ' + 0;
        stat.id = 'spanStat';
        divStat.appendChild(stat);

    },
    setFoodStat(){
        var spanStat = document.getElementById('spanStat');
        spanStat.innerHTML = 'Ваш рекорд: ' + this.foodCount;
    }
}
